...
class Docstrum:
    ...
    def merge_overlapping_blocks(self, components: List[Component], blocks: List[List[List[int]]], 
        ...
        def blocks_are_in_same_line(bounds1, bounds2, vertical_tolerance=0.75):
...
def log_processing_details(filename: str, components: list, text_lines: list, blocks: list, 
    ...
    Args:
        ...
        just_lines: Boolean indicating if only line merging was used
...
def process_and_save_visualization(image: np.ndarray, output_dir: str, filename: str, 
                                 ...
                                 smoothing_arg: int,
                                 line_height: int,  # New parameter for line height
                                 log_file: str):
    """
    ...
    Args:
        [previous args...]
        line_height: Height of the lines to draw (in pixels)
    """
    ...
    # Create final visualization with lines instead of boxes
    vis_image = cv2.cvtColor(image.copy(), cv2.COLOR_GRAY2RGB)
    ...
    for block_idx, block in enumerate(merged_blocks):
        ...
        for line in block:
            ...
            # Get line boundaries
            line_components = sorted(line, key=lambda idx: components[idx].bbox[0])
            
            # Get start and end points of the line
            start_x = components[line_components[0]].bbox[0]
            end_x = components[line_components[-1]].bbox[0] + components[line_components[-1]].bbox[2]
            
            # Calculate vertical position (center of the line)
            y_positions = [components[idx].bbox[1] + components[idx].bbox[3]/2 for idx in line_components]
            y_center = int(np.mean(y_positions))
            
            # Draw a line with specified height
            for h in range(-line_height//2, line_height//2 + 1):
                y = y_center + h
                cv2.line(vis_image, 
                        (start_x, int(y)), 
                        (end_x, int(y)), 
                        color, 
                        1)  # thickness=1 for each line segment
    
    # Save visualizations
    output_path = os.path.join(intermediate_dir, "07_final_lines.png")
    cv2.imwrite(output_path, cv2.cvtColor(vis_image, cv2.COLOR_RGB2BGR))
    
    final_output_path = os.path.join(output_dir, f'{filename}_final_lines.png')
    cv2.imwrite(final_output_path, cv2.cvtColor(vis_image, cv2.COLOR_RGB2BGR))
    
    # Log processing details (same as before)
    ...
    print(f"- Final result: {output_dir}/{filename}_final_lines.png")
...
def main():
    ...
    parser.add_argument('--vertical_distance_threshold', type=float, default=-1,
                       help='Maximum vertical distance between blocks to merge them (default: -1)')
    ...
    parser.add_argument('--big_component_threshold', type=int, default=-1,
                       help='Maximum component size to keep (default: -1)')
    ...
    # Add new argument for line height
    parser.add_argument('--line_height', type=int, default=2,
                       help='Height of the lines to draw in pixels (default: 2)')
    ...
    if args.line_height < 1:
        print("Error: Line height must be a positive integer")
        sys.exit(1)
    ...
    if os.path.isfile(args.input_path):
        ...
        try:
            components, text_lines, orientation, blocks = process_and_save_visualization(
                ...
                args.smoothing_arg,
                args.line_height,  # Pass the new line_height parameter
                args.log_file
            )
        ...
        # Directory processing logic remains the same, just add line_height to process_and_save_visualization calls
        ...
        for filename in os.listdir(args.input_path):
            if os.path.splitext(filename)[1].lower() in image_extensions:
                ...
                if image is None:
                    error_msg = f"Error: Could not load image {image_path}\n"
                    print(error_msg)
                    with open(args.log_file, 'a', encoding='utf-8') as f:
                        f.write(error_msg)
                    errors += 1
        ...
        error_msg = f"Error: {args.input_path} is not a valid file or directory\n"
        print(error_msg)
        with open(args.log_file, 'a', encoding='utf-8') as f:
            f.write(error_msg)
        sys.exit(1)

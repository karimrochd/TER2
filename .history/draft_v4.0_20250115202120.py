...
def process_and_save_visualizations(image: np.ndarray, output_dir: str, filename: str, docstrum: Docstrum):
    """
    Process an image and save all visualizations to files
    
    Args:
        image: Input grayscale image
        output_dir: Directory to save visualizations
        filename: Base filename for saving visualizations
    ...
    ax2.axis('off')
    plt.tight_layout()
    save_visualization(fig, os.path.join(output_dir, f'{filename}_preprocessing.png'))
    ...
    # Find text lines
    text_lines = docstrum.find_text_lines(components, neighbors_info, orientation)
    ...
    plt.title(f'Detected Text Blocks (Orientation: {orientation:.1f}°)')
    plt.axis('off')
    save_visualization(fig, os.path.join(output_dir, f'{filename}_blocks.png'))
...
def main():
    ...
    if os.path.isfile(args.input_path):
        ...
        try:
            components, text_lines, orientation, blocks = process_and_save_visualizations(

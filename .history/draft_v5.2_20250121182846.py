...
class Docstrum:
    ...
    def merge_overlapping_blocks(self, components: List[Component], blocks: List[List[List[int]]], 
                            corner_threshold: float = 20) -> List[List[List[int]]]:
        """
        Modified merge function that preserves small blocks unless they're completely contained
        ...
        def is_contained_within(bounds1, bounds2, padding=5):
            ...
            # Add small padding to avoid merging blocks that just barely touch
            return (x3 - padding <= x1 and x2 <= x4 + padding and 
                    y3 - padding <= y1 and y2 <= y4 + padding)

        def blocks_share_border(bounds1, bounds2):
            """Check if blocks share a border without necessarily having matching corners"""
            if not bounds1 or not bounds2:
                return False
            ...
            # Check if blocks share a significant border
            # Either horizontal or vertical overlap should be significant
            h_overlap_ratio = x_overlap / min(w1, w2) if min(w1, w2) > 0 else 0
            v_overlap_ratio = y_overlap / min(h1, h2) if min(h1, h2) > 0 else 0
            
            # Check if blocks are touching or very close
            touching_horizontally = abs(x1 - x4) <= corner_threshold or abs(x2 - x3) <= corner_threshold
            touching_vertically = abs(y1 - y4) <= corner_threshold or abs(y2 - y3) <= corner_threshold
            
            return ((h_overlap_ratio > 0.3 and touching_vertically) or 
                    (v_overlap_ratio > 0.3 and touching_horizontally))
        ...
        while True:
            ...
            for i in range(len(blocks)):
                ...
                for j in range(i + 1, len(blocks)):
                    ...
                    # Only merge blocks if:
                    # 1. One is completely contained within the other
                    # 2. They share a significant border and are similar in size
                    contained_1_in_2 = is_contained_within(bounds1, bounds2)
                    contained_2_in_1 = is_contained_within(bounds2, bounds1)
                    
                    if contained_1_in_2 or contained_2_in_1:
                        # Merge the contained block into the containing block
                        if contained_1_in_2:
                            blocks[j].extend(blocks[i])
                            blocks[j].sort(key=lambda line: min(components[idx].centroid[1] 
                                                            for idx in line))
                            blocks.pop(i)
                        else:
                            blocks[i].extend(blocks[j])
                    ...
                    # Check if blocks share a border and are similar in size
                    elif blocks_share_border(bounds1, bounds2):
                        # Calculate block sizes
                        size1 = (bounds1[2] - bounds1[0]) * (bounds1[3] - bounds1[1])
                        size2 = (bounds2[2] - bounds2[0]) * (bounds2[3] - bounds2[1])
                        
                        # Only merge if blocks are similar in size (within factor of 3)
                        if 0.33 <= size1/size2 <= 3.0:
                            blocks[i].extend(blocks[j])
                            blocks[i].sort(key=lambda line: min(components[idx].centroid[1] 
                                                            for idx in line))
                            blocks.pop(j)

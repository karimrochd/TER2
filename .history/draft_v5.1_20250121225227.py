...
class Docstrum:
    def __init__(self, k_nearest: int = 5, angle_threshold: float = 30, min_component_size: int = 7):
        """
        ...
        Args:
            ...
            min_component_size: Minimum size for connected components (default 7)
        """
        self.k = k_nearest
    ...
    def preprocess(self, image: np.ndarray) -> np.ndarray:
        ...
        binary = remove_small_components(binary, k=self.min_component_size)
    ...
    def find_nearest_neighbors(self, components: List[Component]) -> List[List[Tuple[int, float, float]]]:
        """
        Find k nearest neighbors for each component with improved angle calculation
        ...
        for i, (component_neighbors, neighbor_distances) in enumerate(zip(indices, distances)):
            ...
            neighbors = []
            for j, (neighbor_idx, dist) in enumerate(zip(component_neighbors[1:], neighbor_distances[1:]), 1):
                # Calculate angle between components with improved angle calculation
                dx = points[neighbor_idx][0] - points[i][0]
                dy = points[neighbor_idx][1] - points[i][1]
                
                # Calculate raw angle and normalize to [0, 180)
                angle = np.degrees(np.arctan2(dy, dx)) % 180
                
                # Snap angles to horizontal/vertical when close
                if min(angle, abs(angle - 180)) < self.angle_threshold:
                    angle = 0
    ...
    def estimate_orientation(self, neighbors_info: List[List[Tuple[int, float, float]]]) -> float:
        """
        Estimate document orientation from neighbor angles with improved angle detection
        ...
        Returns:
            Estimated orientation angle in degrees
        """
        # Collect angles and their weights
        angles = []
        weights = []
        for component_neighbors in neighbors_info:
            for _, dist, angle in component_neighbors:
                angles.append(angle)
                # Use inverse distance as weight (closer connections matter more)
        ...
        # Create weighted histogram with more bins for better precision
        hist, bins = np.histogram(angles, bins=180, range=(0, 180), weights=weights)
        
        # Apply Gaussian smoothing
        sigma = 2.0
        window = 11  # Should be odd
        x = np.linspace(-3*sigma, 3*sigma, window)
        ...
        orientation = bins[peak_idx]
        
        # Check horizontal vs vertical orientation strength
        horizontal_range = np.logical_or(bins[:-1] < 10, bins[:-1] > 170)
        vertical_range = np.logical_and(bins[:-1] > 80, bins[:-1] < 100)
        
        horizontal_strength = np.sum(hist_smooth[horizontal_range])
        vertical_strength = np.sum(hist_smooth[vertical_range])
        
        # Adjust orientation based on dominant direction
        if vertical_strength > 2 * horizontal_strength:
            orientation = 90.0
        elif horizontal_strength > 2 * vertical_strength:
            orientation = 0.0
    ...
    def check_border_proximity(bounds1, bounds2, border_threshold):
        """
        Check if any borders of two blocks are close enough to be merged
        
        Args:
            bounds1: (x1, y1, x2, y2) of first block
            bounds2: (x1, y1, x2, y2) of second block
            border_threshold: Maximum distance between borders to consider them close
            
        Returns:
            bool: True if borders are close enough to merge
        """
        ...
        # Check vertical borders (left and right edges)
        left_dist = min(abs(x1 - x3), abs(x1 - x4))
        right_dist = min(abs(x2 - x3), abs(x2 - x4))
        
        # Check horizontal borders (top and bottom edges)
        top_dist = min(abs(y1 - y3), abs(y1 - y4))
        bottom_dist = min(abs(y2 - y3), abs(y2 - y4))
        
        # Check if any borders are close enough
        if min(left_dist, right_dist, top_dist, bottom_dist) <= border_threshold:
            # Additional check: ensure there's some vertical or horizontal overlap
    ...
    def merge_overlapping_blocks(self, components: List[Component], blocks: List[List[List[int]]], 
                                overlap_threshold: float = 0.00001,
                                alignment_threshold: float = 0.25,
                                corner_threshold: float = 20,
                                border_threshold: float = 15) -> List[List[List[int]]]:
        """
        Merge blocks that:
        ...
        4. Have borders that are close to each other within border_threshold
        ...
        Args:
            ...
            border_threshold: Maximum distance between borders to consider them close enough to merge
        """
        ...
        while True:
            ...
            for i in range(len(blocks)):
                ...
                for j in range(i + 1, len(blocks)):
                    ...
                    borders_close = check_border_proximity(bounds1, bounds2, border_threshold)
                    ...
                    if ((h_overlap > overlap_threshold and h_aligned) or 
                        ...
                        is_contained or shares_border or borders_close):
                        # Merge blocks
...
def process_and_save_visualization(image: np.ndarray, output_dir: str, filename: str, 
                                 docstrum: Docstrum, spacing_factor: float, corner_threshold: float):
    """
    ...
    merged_blocks = docstrum.merge_overlapping_blocks(components, initial_blocks, corner_threshold=corner_threshold)
...
def main():
    ...
    parser.add_argument('--min_component_size', type=int, default=7,
                       help='Minimum size for connected components to retain (default: 7)')
    parser.add_argument('--border_threshold', type=float, default=15,
                       help='Maximum distance between borders to consider them close enough to merge (default: 15)')
    # Parse arguments
    ...
    if os.path.isfile(args.input_path):
        ...
        try:
            ...
            args.spacing_factor, args.corner_threshold, args.border_threshold
        )
        ...
        for filename in os.listdir(args.input_path):
            if os.path.splitext(filename)[1].lower() in image_extensions:
                ...
                try:
                    components, text_lines, orientation, blocks = process_and_save_visualization(
                        image, args.output_dir, base_filename, docstrum, args.spacing_factor, args.corner_threshold
                    )
                    print(f"Processed {filename}:")
                    print(f"- Found {len(components)} components")
                    print(f"- Grouped into {len(text_lines)} text lines")

...
def visualize_preprocessing(image: np.ndarray, binary: np.ndarray, output_dir: str, filename: str):
    """
    Visualize and save the preprocessing step sbinary image
    """
...
def visualize_neighbors(image: np.ndarray, components: List[Component], 
    ...
    plt.imshow(vis_image)
    # add colorbar
    sm = plt.cm.ScalarMappable(cmap=plt.cm.hsv, norm=plt.Normalize(vmin=0, vmax=180))
    sm.set_array([])
    plt.colorbar(sm, ticks=np.linspace(0, 180, num=7), label='Angle (degrees)')

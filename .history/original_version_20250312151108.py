...
class Docstrum:
    ...
    def preprocess(self, image: np.ndarray, kfill_threshold = 5) -> np.ndarray:
        """
        ...
        binary = kfill(binary, k= kfill_threshold, max_iterations=10)
    ...
    def process(self, image: np.ndarray, kfill_threshold = 5):
        """
        ...
        """
        # Preprocess image
        binary = self.preprocess(image, kfill_threshold = kfill_threshold)
...
def main():
    ...
    if os.path.isfile(args.input_path):
        ...
        components, text_lines, orientation, blocks, spacings, binary, initial_orientation, neighbors_info = docstrum.process(image, kfill_threshold = args.kfill_threshold)
        ...
        elif args.visualization == 'standard':
            # Standard visualization - just blocks and summary
            docstrum.visualize_preprocessing(image, binary, args.output_dir, filename)
            docstrum.visualize_text_lines(image, components, text_lines, args.output_dir, filename)
            docstrum.visualize_blocks(image, components, blocks, args.output_dir, filename)
        else:  # 'minimal'
        ...
        for filename in os.listdir(args.input_path):
            if os.path.splitext(filename)[1].lower() in image_extensions:
                ...
                try:
                    ...
                    components, text_lines, orientation, blocks, spacings, binary, initial_orientation, neighbors_info = docstrum.process(image, args.kfill_threshold)
                    ...
                    if args.visualization == 'detailed':
                        ...
                        # Standard visualization - just blocks and summary
                        docstrum.visualize_preprocessing(image, binary, args.output_dir, base_filename)
                        docstrum.visualize_text_lines(image, components, text_lines, args.output_dir, base_filename)
                        docstrum.visualize_blocks(image, components, blocks, args.output_dir, base_filename)
                    else:  # 'minimal'
                        # Minimal visualization - just blocks
                        docstrum.visualize_blocks(image, components, blocks, args.output_dir, base_filename)

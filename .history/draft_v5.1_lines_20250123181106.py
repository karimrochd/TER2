...
class Docstrum:
    ...
    def find_text_lines(self, components: List[Component], 
                        ...
                        spacing_factor: float = 1.2) -> List[List[int]]:
        """
        Modified version to be more strict about line separation
        """
        def get_vertical_distance(comp1: Component, comp2: Component) -> float:
            """Calculate vertical distance between components' center points"""
            return abs(comp1.centroid[1] - comp2.centroid[1])

        def get_height(comp: Component) -> float:
            """Get component height"""
            return comp.bbox[3]  # height is the 4th element of bbox

        def is_same_line(comp1: Component, comp2: Component) -> bool:
            """
            Determine if two components should be considered on the same line.
            Much stricter criteria for same-line membership.
            """
            vert_dist = get_vertical_distance(comp1, comp2)
            avg_height = (get_height(comp1) + get_height(comp2)) / 2
            
            # Components must have very similar vertical positions
            # Using a stricter threshold of 0.3 times the average height
            return vert_dist < (0.3 * avg_height)

        # Initialize lines
        remaining_components = set(range(len(components)))
        text_lines = []

        while remaining_components:
            # Start a new line with the leftmost remaining component
            current_line = set()
            start_idx = min(remaining_components, 
                        key=lambda idx: components[idx].centroid[0])
            current_line.add(start_idx)
            remaining_components.remove(start_idx)
            
            # Keep track of the current line's reference component for vertical position
            reference_comp = components[start_idx]

            # Find all components that belong to this line
            while True:
                line_extended = False
                
                # Find the next component that could be part of this line
                candidates = []
                for idx in remaining_components:
                    if is_same_line(reference_comp, components[idx]):
                        candidates.append(idx)
                
                if candidates:
                    # Add the leftmost candidate
                    next_idx = min(candidates, 
                                key=lambda idx: components[idx].centroid[0])
                    current_line.add(next_idx)
                    remaining_components.remove(next_idx)
                    line_extended = True
                
                if not line_extended:
                    break

            # Convert line to list and sort by x-coordinate
            current_line = sorted(list(current_line), 
                                key=lambda idx: components[idx].centroid[0])
            text_lines.append(current_line)

        # Sort lines by y-coordinate
        text_lines.sort(key=lambda line: min(components[idx].centroid[1] 
                                        for idx in line))
    ...
    def merge_overlapping_blocks(self, components: List[Component], blocks: List[List[List[int]]], 
                            ...
                            corner_threshold: float = 20) -> List[List[List[int]]]:
        """
        In this modified version, we don't merge blocks since we want to keep lines separate
        ...
        Returns:
            The same blocks without any merging
        """
        return blocks
...
def main():
    ...
    # Add spacing_factor and corner_threshold arguments
    ...
    if os.path.isfile(args.input_path):
        ...
        try:
            components, text_lines, orientation, blocks = process_and_save_visualization(
                image, args.output_dir, filename, docstrum, args.spacing_factor, args.corner_threshold
            )

...
def remove_small_components(binary_image, small_components_threshold = 7):
    """
    ...
    for i in range(1, num_labels):  # Skip the background label (0)
        if stats[i, cv2.CC_STAT_AREA] >= small_components_threshold:
...
class Docstrum:
    ...
    def preprocess(self, image: np.ndarray, small_components_threshold = 7) -> np.ndarray:
        """
        ...
        binary = remove_small_components(binary, small_components_threshold= small_components_threshold)
        
        return binary

    def find_connected_components(self, binary: np.ndarray, big_components_threshold : int) -> List[Component]:
        """
        ...
        for i in range(1, num_labels):
            ...
            if big_components_threshold == -1:
                if area < median_area * 0.05:
                    continue
            else:
                if area < median_area * 0.05 or area > big_components_threshold:
                    continue
...
def process_and_save_visualization(image: np.ndarray, output_dir: str, filename: str, 
                                 ...
                                 small_components_threshold : int,
                                 big_components_threshold : int,
    ...
    binary = docstrum.preprocess(image, small_components_threshold)
    components = docstrum.find_connected_components(binary, big_components_threshold)
...
def main():
    ...
    parser.add_argument('--small_components_threshold', type=int, default=7,
                          help='Minimum size of small components to remove (default: 7 pixels)')
    parser.add_argument('--big_components_threshold', type=int, default=-1,
                          help='Maximum size of big components to remove (default: -1 (no removal))')
    ...
    if os.path.isfile(args.input_path):
        ...
        try:
            components, text_lines, orientation, blocks = process_and_save_visualization(
                ...
                args.just_lines,
                args.small_components_threshold,
                args.big_components_threshold
            )
